...
Solo aprueba cuando el documento y sus afirmaciones son demostrables en el
Normograma DIAN. La auditoría no escribe; --apply escribe explícitamente.
"""
...
def date_candidates(value):
    s=str(value or "").strip(); out={norm(s)} if s else set()
    m=re.fullmatch(r"(\d{4})-(\d{2})-(\d{2})",s)
    if m:
        y,mo,d=map(int,m.groups())
        if 1<=mo<=12:
            month=next(k for k,v in MESES.items() if v==mo)
            out.update({norm(f"{d} de {month} de {y}"),norm(f"{d}/{mo:02d}/{y}"),norm(f"{d:02d}/{mo:02d}/{y}")})
    return out

def evidence(source_text,value):
    if value in (None,"",[]): return False
    src=norm(source_text)
    candidates=date_candidates(value) or {norm(value)}
    for c in candidates:
...
def extract_claim_tokens(sentence):
    # Excluye palabras funcionales y exige anclas verificables para evitar que
    # la similitud de texto premie resúmenes vagos.
...
def check_summary(source_text,summary):
    ...
    for i,sentence in enumerate(sentences,1):
        ...
        # Los números del resumen deben existir literalmente en la fuente;
        # las fechas admiten representaciones equivalentes.
        for token in re.findall(r"\b\d+(?:[.,]\d+)?%?|\b(?:19|20)\d{2}\b",norm(sentence)):
            ...
            elif token not in src: errors.append(f"Afirmación {i}: el dato '{token}' no aparece en la fuente.")
...
def verify(session,doc):
    ...
    fpdate=str(doc.get("fecha_publicacion") or "")
    if not doc.get("fecha_es_real"):errors.append("La fecha de publicación está marcada como sintética/no demostrada.")
    elif not evidence(source,fpdate):errors.append("La fecha de publicación no aparece en formato demostrable.")
...
def iter_docs(db,limit=None):
    ...
    while True:
        ...
        if size<=0:return
        rows=(db.table("documentos_tributarios").select("id,numero_resolucion,contenido,descripcion_limpia,resumen_humano,resumen_borrador,enlace_oficial,fecha_publicacion,fecha_es_real,tesis_juridica,entidad_emisora,estado_vigencia").not_.is_("resumen_borrador","null").order("id").range(offset,offset+size-1).execute().data or [])
        if not rows:return
        for row in rows:
...
def main(apply=False,limit=None):
    ...
    db=create_client(u,k); session=requests.Session(); session.headers.update({"User-Agent":"EUCLIDIAN/1.3 (verificador de alta confianza)","Accept-Language":"es-CO,es;q=0.9"}); validate_sources(session)
