...
def numero_y_anio(texto:str)->tuple[str|None,int|None]:
    patrones=[r"(?:CONCEPTO|OFICIO)[-\s]+(?:A)?(\d{3,6})[-\s]+(?:int\.?[-\s]*\d+[-\s]+)?(?:DE[-\s]+)?((?:19|20)\d{2})",r"(?:CONCEPTO|OFICIO)[-\s]+(?:A)?(\d{3,6})[-\s]+int[-\s]*\d+[-\s]+\d{4}(\d{4})"]
    for p in patrones:
...
def main()->int:
    ...
    r=s.get(URL,timeout=30);r.raise_for_status();soup=BeautifulSoup(r.text,"html.parser");db=create_client(u,k);rows=[]
    for tr in soup.find_all("tr"):
        texto=" ".join(tr.stripped_strings);numero,anio=numero_y_anio(texto)
        if not numero or not anio:continue
        a=tr.find("a",href=True)
        if not a:continue
        href=urljoin(URL,a["href"])
        if ".pdf" not in href.lower():continue
        celdas=tr.find_all(["th","td"]);tema=celdas[2].get_text(" ",strip=True) if len(celdas)>2 else "";tesis=celdas[3].get_text(" ",strip=True) if len(celdas)>3 else "";fecha=parse_fecha(texto,anio)
        rows.append({"numero_resolucion":f"DIAN-OFICIO-{numero}-{anio}","tipo_documento":"concepto" if "CONCEPTO" in texto.upper() else "oficio","titulo":a.get_text(" ",strip=True) or f"Documento DIAN {numero} de {anio}","enlace_oficial":NORMOGRAMA.format(numero=numero,anio=anio),"fecha_publicacion":fecha,"fecha_es_real":bool(fecha),"anio_publicacion":anio,"descripcion_limpia":tesis[:4000] if tesis else tema[:4000],"materia":tema[:40] if tema else None,"publicado_cliente":False,"revisado_por_humano":False,"aprobado_para_email":False,"revisado_fiscal_en":None})
    unique={x["numero_resolucion"]:x for x in rows}
    if unique:db.table("documentos_tributarios").upsert(list(unique.values()),on_conflict="numero_resolucion").execute()
