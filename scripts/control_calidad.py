...
superan las comprobaciones de integridad. Ante duda, se bloquea.
"""
...
class Control:
    ...
    def _aprobados(self):
        ...
        except Exception as e:self.graves.append(f"No se pudo revisar aprobados: {str(e)[:150]}"); return
    ...
    def _fechas(self):
        try:
            hoy=date.today().isoformat(); r=self.db.table("documentos_tributarios").select("numero_resolucion,fecha_publicacion").gt("fecha_publicacion",hoy).limit(20).execute().data or []
    ...
    def _veredicto(self):
        ...
        if self.graves or (self.estricto and self.avisos):
            log.error("NO APTO PARA ENVÍO"); return 1
