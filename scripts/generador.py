...
class Generador:
    def __init__(self, desde=None, hasta=None, maximo=6, dias=7):
        if not SUPABASE_URL or not SUPABASE_KEY:
            log.error("Faltan SUPABASE_URL o SUPABASE_SERVICE_KEY")
            sys.exit(1)
        self.db = create_client(SUPABASE_URL, SUPABASE_KEY)
        self.hasta = hasta or date.today()
        # La ventana por defecto es una semana, que es el ritmo del
        # boletin. Pero al arrancar hay aprobados mas viejos, y sin una
        # ventana amplia el generador no encuentra nada y no deja
        # borrador. De ahi la opcion --dias.
        self.desde = desde or (self.hasta - timedelta(days=dias))
        self.maximo = maximo
        self.stats = Counter()
    ...
    def correr(self, guardar=False):
        ...
        if not docs:
            log.warning("No hay aprobados entre %s y %s.",
                        self.desde, self.hasta)
            # Distinguir "no aprobaste nada" de "aprobaste, pero fuera de
            # la ventana" evita mandar a alguien a la bandeja por nada.
            try:
                ...
                if total.data:
                    log.warning("")
                    log.warning("Si hay %d aprobados, pero con fecha anterior:",
                                len(total.data))
                    for t in total.data[:6]:
                        log.warning("   %s  (%s)", t["numero_resolucion"],
                                    t["fecha_publicacion"])
                    log.warning("")
                    log.warning("Amplia la ventana con --dias 365")
                else:
    ...
    def _asunto(self, docs):
        """
        El asunto tiene que decir la verdad sobre el periodo. Decir "de
        esta semana" cuando los documentos son de marzo y julio es una
        falsedad pequena, pero en un boletin cuyo valor es la exactitud
        ...
        if fechas:
            try:
                menor = min(date.fromisoformat(str(f)[:10]) for f in fechas)
                mayor = max(date.fromisoformat(str(f)[:10]) for f in fechas)
                dias = (self.hasta - menor).days
                if dias > 40:
                    if menor.year == mayor.year and menor.month != mayor.month:
                        periodo = f"de {MESES[menor.month]} a {MESES[mayor.month]}"
                    elif menor.year != mayor.year:
                        periodo = f"de {menor.year} a {mayor.year}"
                    else:
                        periodo = f"de {MESES[menor.month]}"
                elif dias > 9:
                    periodo = "reciente" if n == 1 else "recientes"
  ...
  <div style="font-family:'Courier New',monospace;font-size:10px;color:{TENUE};
    ...
    def _html_ficha(self, d, alerta, i):
      ...
      <div style="background:{fondo};border-left:3px solid {borde};
        ...
        extras = []
        # Lo primero que debe ver el lector: si una norma posterior ya
        # toco esta. Aplicar doctrina superada es el error mas caro.
        despues = d.get("modificado_por") or []
        if despues:
            nums = ", ".join(str(x.get("numero", "")) for x in despues[:3])
...
if __name__ == "__main__":
    ...
    ap.add_argument("--dias", type=int, default=7,
                    help="Cuantos dias hacia atras buscar aprobados")
