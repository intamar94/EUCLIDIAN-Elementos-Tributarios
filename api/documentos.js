...
const POR_PAGINA = 25;
const ESTADOS = {
  pendientes: 'revisado_fiscal_en=is.null',
  nuevos: 'es_nuevo=is.true',
  aprobados: 'revisado_fiscal_en=not.is.null&aprobado_para_email=is.true',
  todos: '',
};
...
const ORDENES = {
  recientes: 'fecha_publicacion.desc.nullslast,fecha_publicacion_web.desc.nullslast,numero_resolucion.desc',
  prioridad: 'orden_prioridad.asc,fecha_publicacion.desc.nullslast,fecha_publicacion_web.desc.nullslast',
  antiguos: 'fecha_publicacion.asc.nullslast,fecha_publicacion_web.asc.nullslast,numero_resolucion.asc',
};
const CAMPOS = [
  'id','numero_resolucion','numero_interno','tipo_documento','contenido','descripcion_limpia','titulo',
...
export default async function handler(req,res){
  ...
  try{
    ...
    if(documentos.length){
      ...
      if(rEval.ok){
        const evaluaciones=await rEval.json();
        const porId=new Map(evaluaciones.map(e=>[e.documento_id,e]));
        for(const d of documentos){
          const e=porId.get(d.id);
          if(e){
            d.evaluacion_resultado=e.resultado||null;
            d.evaluacion_puntuacion=e.puntuacion??null;
            d.evaluacion_reglas_pasadas=e.reglas_pasadas||[];
            d.evaluacion_reglas_fallidas=e.reglas_fallidas||[];
            d.evaluacion_motivos=(e.motivos||[]).map(normalizarDiagnostico);
    ...
    const rResumen=await fetch(`${SUPABASE_URL}/rest/v1/rpc/conteos_bandeja`,{method:'POST',headers:{...cabeceras,'Content-Type':'application/json'},body:JSON.stringify({p_periodo:periodo,p_tema:tema||null,p_estado:estado,p_prioridad:prioridad||null,p_naturaleza:naturaleza||null})});
