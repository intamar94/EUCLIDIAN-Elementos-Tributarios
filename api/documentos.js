...
export default async function handler(req,res){
  ...
  try{
    ...
    const prioridadConteos={importante:0,informativa:0},naturalezaConteos={obligatorias:0,conceptos:0};
    documentos.forEach(d=>{if(d.prioridad==='importante')prioridadConteos.importante++;if(d.prioridad==='informativa')prioridadConteos.informativa++;if(d.clasificacion_obligatoriedad==='obligatorio_dian_y_contribuyentes')naturalezaConteos.obligatorias++;if(d.clasificacion_obligatoriedad==='obligatorio_dian_solo')naturalezaConteos.conceptos++;});
