...
/* La fecha, con su precision declarada.
 *
 * De los 17.595 documentos, unos traen el dia exacto leido del texto
 * oficial y otros solo el año, tomado del numero de la norma. Mostrar
 * "1 ene 2019" en el segundo caso seria inventar un dia que nadie
 * verifico. Se muestra el año a secas, que es lo que sabemos. */
function fechaFicha(d){
  ...
  const anio = d.anio || String(d.fecha_publicacion || '').slice(0,4);
  if (!anio) return '';
  return `<span class="fecha aproximada" title="La DIAN no publicó el día exacto en el listado">${anio}</span>`;
}
