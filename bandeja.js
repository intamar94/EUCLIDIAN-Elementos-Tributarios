/* EUCLIDIAN — navegación de la bandeja del cliente.
 * El cliente consulta únicamente información ya verificada/publicada.
 * El estado interno de revisión no forma parte de esta interfaz.
 */
let CLAVE = sessionStorage.getItem('euclidian_clave') || '';
const F = { estado:'aprobados', prioridad:'', naturaleza:'', tema:'', orden:'recientes', pagina:1 };
...
async function cargar(){
  ...
  try{
    const q=new URLSearchParams({estado:'aprobados',orden:F.orden,pagina:F.pagina});
    ...
    const data=await r.json();if(!r.ok)throw new Error(data.detalle||data.error);
    document.getElementById('puerta').hidden=true;document.getElementById('mal').textContent='';document.getElementById('cab').hidden=false;document.getElementById('controles').hidden=false;
    pintar('gPrioridad',data.prioridad||{});pintar('gNaturaleza',data.naturaleza||{});marcarActivos();poblarTemas(data.temas||[]);
    const s=document.getElementById('sello');if(s&&data.actualizado){const f=new Date(data.actualizado);s.textContent='Información actualizada el '+f.toLocaleDateString('es-CO',{day:'numeric',month:'long'})+', '+f.toLocaleTimeString('es-CO',{hour:'2-digit',minute:'2-digit'});}
    if(!data.documentos.length){lista.innerHTML='<div class="aviso"><b>No hay resultados con estos filtros.</b><span>Prueba con otro criterio de búsqueda.</span></div>';return;}
    lista.innerHTML=data.documentos.map(ficha).join('');paginacion(data);window.scrollTo({top:0,behavior:'smooth'});
  }catch(e){lista.innerHTML='<div class="error"><b>No se pudo cargar la información.</b><span>Inténtalo de nuevo en unos momentos.</span></div>';}
...
function grupoclic(grupo,campo){grupoClic(grupo,campo);}
grupoclic('gNaturaleza','naturaleza');
document.getElementById('selTema').addEventListener('change',e=>{F.tema=e.target.value;F.pagina=1;cargar();});
document.getElementById('selOrden').addEventListener('change',e=>{F.orden=e.target.value;F.pagina=1;cargar();});
